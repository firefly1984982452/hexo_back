...
宏任务：主代码、setTimeout、setInterval
微任务：promise
...
var obj = {
  ...
  fun: function(){
    console.log(this.name+this.age+this.objAge) // 此处的this指向obj，所以显示'张三undefined undefined'
  }
}
```
...
```
// 防抖【防止多次触发滚动事件】
handleScrollThrottle (fn, delay) {
  //1s调用一次。上次的任务还在，结束上次任务，
